...
document.querySelectorAll('#map-stage .marker').forEach(m => {
  m.addEventListener('click', () => {
    ...
    const rainEl = document.getElementById('sp-rain');
    if (rainEl) rainEl.textContent = m.dataset.rain === '—' ? '—' : m.dataset.rain + ' mm';
    const soilEl = document.getElementById('sp-soil');
    if (soilEl) soilEl.textContent = m.dataset.soil === '—' ? '—' : m.dataset.soil + '%';
    const roadEl = document.getElementById('sp-road');
    if (roadEl) roadEl.textContent = m.dataset.road;
    const aiEl = document.getElementById('sp-ai');
    if (aiEl) aiEl.textContent = m.dataset.ai;
    const chip = document.getElementById('sp-risk');
    if (chip && m.dataset.risk && riskLabel[m.dataset.risk]) {
